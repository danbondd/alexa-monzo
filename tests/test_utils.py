...
class TestUtils(object):
    ...
    def setUp(self):
        with open('./fixtures/transactions.json') as data:
            TestUtils.transactions = json.load(data)


    def test_calculate_total(self):
        expected = 29099

        total = utils.calculate_total(TestUtils.transactions['transactions'], None)
    ...
    def test_calculate_total_with_category(self):
        ...
        for category, expected in results.items():
            total = utils.calculate_total(TestUtils.transactions['transactions'], category)
            assert_equal(total, expected)
    ...
    def test_speakable_currency(self):
        ...
        for pence, expected in money.items():
    ...
    def test_duration_to_words(self):
        ...
        for duration, expected in durations.items():
